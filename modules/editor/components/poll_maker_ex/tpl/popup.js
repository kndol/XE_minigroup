...
jQuery(function($){
	...
	if($node.length && $node.attr('editor_component') == 'poll_maker_ex') {
		alert(msg_poll_cannot_modify);
		return window.close();
	}

	// 옵션 체크 버튼
	$('#opt_show_result').click(function(){
		$('#opt_voter_result').prop('disabled', !this.checked);
		$('#opt_expire_result').prop('disabled', !this.checked);
		$('#opt_show_voters').prop('disabled', !this.checked);
	});
