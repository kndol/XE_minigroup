...
class jsonLib extends json {
	function encrypt($security_key, $jsonData) {
		return $this->stringToHex($this->des($security_key, json_encode($jsonData), 1));
	}
	
	function decrypt($security_key, $hexStr) {
		return json_decode(trim(mcrypt_decrypt(MCRYPT_DES, $security_key, $this->hexToString($hexStr), MCRYPT_MODE_ECB)));
	}
	...
	function hexToString($h) {
		$r = "";
		for ($i=(substr($h, 0, 2)=="0x")?2:0; $i < strlen($h)-1; $i+=2) $r .= chr(hexdec($h[$i].$h[$i+1]));
		return $r;
	}
}
/* End of file json.lib.php */
/* Location: ./modules/json/json.lib.php */
