function minigroupLoadMenuInfo(url){
	var $ = jQuery;
    ...
    $.get(url,function(data){
        $(data).find("node").each(function(i){
            ...
            $('<a href="#__menu_info" class="modify modalAnchor"><img src="./common/js/plugins/ui.tree/images/iconModify.gif" /></a>').bind("click",function(e){
				$("#tree_"+node_srl+" > span").click();
                minigroupModifyNode(node_srl,e);
                return false;
        ...
        simpleTreeCollection = $('.simpleTree').simpleTree({
            ...
            afterMove:function(destination, source, pos){
                ...
                $.exec_json("minigroup.procMinigroupMenuItemMove",{ "menu_srl":menu_srl,"parent_srl":parent_srl,"target_srl":target_srl,"source_srl":source_srl,"mode":mode},
                function(data){
                    if(data.error>0){
                        minigroupLoadMenuInfo(xml_url);
                    }
...
function minigroupAddMenu(node_srl,e) {
    ...
    jQuery.exec_json('minigroup.getMinigroupMenuTplInfo', params, function(data){
        jQuery('#menu_zone_info').html(data.tpl);
    });
}

function minigroupModifyNode(node_srl,e){
    jQuery('#menu_zone_info').html('');
    jQuery("#tree_"+node_srl+" > span").click();
...
function minigroupDeleteMenu(node_srl) {
